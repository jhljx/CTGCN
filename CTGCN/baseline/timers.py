...
def get_sp_adj_mat(file_path, node2idx_dict):
...
def get_sp_delta_adj_mat(A_pre, file_path, node2idx_dict):
    N = len(list(node2idx_dict.keys()))
    A_cur = get_sp_adj_mat(file_path, node2idx_dict)
    ...
    pre_dict = dict(zip(zip(pre_row.tolist(), pre_col.tolist()), pre_value.tolist()))
    cur_dict = dict(zip(zip(cur_row.tolist(), cur_col.tolist()), cur_values.tolist()))
...
def timers(nodes_file, input_path, output_path, Theta=0.17, dim=128, Update=True):
    # nodes set
    nodes_set = pd.read_csv(nodes_file, names=['node'])
    full_node_list = nodes_set['node'].tolist()
    N = len(full_node_list)
    print('node num: ', N)
    node2idx_dict = dict(zip(full_node_list, np.arange(N).tolist()))
    # base graph adjlist
    f_list = sorted(os.listdir(input_path))
    f0 = os.path.join(input_path, f_list[0])
    print(f0)
    A = get_sp_adj_mat(f0, node2idx_dict)
    ...
    result.to_csv(os.path.join(output_path, f_list[0]), sep='\t')
    ...
    for i in range(1, time_slice + 1):
        ...
        fn = os.path.join(input_path, f_list[i])
        S_add = get_sp_delta_adj_mat(S_cum, fn, node2idx_dict)
        ...
        assert output_data.shape[0] == N
        result = pd.DataFrame(data=output_data, index=full_node_list, columns=range(2 * dim))
        result.to_csv(os.path.join(output_path, f_list[i]), sep='\t')
