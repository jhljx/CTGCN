...
from RWTGCN.models import GCN, RWTGCN
...
def rwtgcn_embedding(dataset, learning_type='unsupervise'):
    base_path = os.path.abspath(os.path.join(os.getcwd(), '../..', 'data/' + dataset + '/RWT-GCN'))
    origin_folder = os.path.join('..', '1.format')
    origin_base_path =  os.path.abspath(os.path.join(base_path, origin_folder))
    embedding_folder = os.path.join('..', '2.embedding/RWTGCN')
    core_folder = 'rwtgcn_cores'
    core_base_path = os.path.abspath(os.path.join(base_path, core_folder))
    structure_info_folder = 'rwtgcn_structure_info'
    structure_based_path = os.path.abspath(os.path.join(base_path, structure_info_folder))
    node_file = os.path.join('..', 'nodes_set/nodes.csv')
    ...
    t1 = time.time()
    print('start RWTGCN embedding on ' + dataset)
    if learning_type == 'unsupervise':
        ...
        for idx in range(0, max_time_num, duration - 1):
            ...
            # x_list, max_degree, degree_list = data_loader.get_degree_feature_list(origin_base_path, start_idx=idx, duration=duration)
            # print('degree list shape: ', degree_list[0].shape)
            ...
            # output_dim_list, structure_info_list = data_loader.get_structure_info_list(structure_based_path, start_idx=idx, duration=duration)

            rwtgcn_model = RWTGCN(input_dim=node_num, hidden_dim=500, output_dim=128, layer_num=1, duration=time_num, bias=True, rnn_type='GRU')
            ...
            rwtgcn.learn_embedding(adj_list, x_list, single_output=False, epoch=50, batch_size=4096 * 8, lr=0.001,
                                   start_idx=idx, weight_decay=5e-4, model_file='rwtgcn', export=True)
    else:
        label_file = os.path.join('..', 'nodes_set/labels.csv')
        label_path = os.path.abspath(os.path.join(base_path, label_file))
        df_label = pd.read_csv(label_path, names=['label'])
        ...
        for idx in range(0, max_time_num, duration):
            ...
            # output_dim_list, structure_info_list = data_loader.get_structure_info_list(structure_based_path, start_idx=idx, duration=duration)
            rwtgcn_model = RWTGCN(input_dim=node_num, hidden_dim=128, output_dim=128, layer_num=1, duration=duration, bias=True, rnn_type='GRU')
            ...
            rwtgcn = SupervisedEmbedding(base_path=base_path, origin_folder=origin_folder, embedding_folder=embedding_folder,
                                            node_list=nodes_set['node'].tolist(), model=rwtgcn_model,
                                            loss=rwtgcn_loss, max_time_num=max_time_num)
            rwtgcn.learn_embedding(adj_list, x_list, structure_info_list=structure_info_list, epoch=50, batch_size=4096 * 8, lr=0.001, start_idx=idx,
                                      weight_decay=5e-4, model_file='rwtgcn', export=True)
...
if __name__ == '__main__':
    dataset = 'facebook'
    rwtgcn_embedding(dataset=dataset)