...
class DataGenerator(object):
    ...
    def __init__(self, base_path, input_folder, output_folder, node_file, label_file, sep=' ', test_ratio=0.1, val_ratio=0.2):
        ...
        node_idx_list = df_label['node'].apply(lambda x: node2idx_dict[x]).tolist()
        # print(node_idx_list)
        df_label.index = node_idx_list
...
class NodeClassifier(object):
    ...
    def train(self, train_nodes, val_nodes, embeddings):
        ...
        for C in [0.01, 0.1, 1, 5, 10, 20]:
            model = LogisticRegression(C=C, solver='lbfgs', max_iter=5000, class_weight='balanced', multi_class='ovr')
    ...
    def node_classification_all_time(self, method):
        ...
        for i, f_name in enumerate(f_list):
            ...
            model_dict = self.train(train_nodes, val_nodes, embeddings)
            acc_list = self.test(test_nodes, embeddings, model_dict, date)
...
if __name__ == '__main__':
    ...
    data_generator = DataGenerator(base_path="../../data/" + dataset, input_folder="1.format",
                                   output_folder="node_classification_data", node_file="nodes_set/nodes.csv", label_file="nodes_set/labels.csv")
    ...
    node_classifier = NodeClassifier(base_path="../../data/" + dataset, origin_folder='1.format', embedding_folder="2.embedding",
                                   nodeclas_folder="node_classification_data", output_folder="node_classification_res", node_file="nodes_set/nodes.csv")
    ...
    t1 = time.time()
    node_classifier.node_classification_all_method(method_list=method_list, worker=-1)
    t2 = time.time()
