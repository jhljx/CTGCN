...
class Processing:
    def __init__(self, base_path, origin_folder, structure_folder, core_folder, walk_pair_folder, node_freq_folder, node_file, hop=5, walk_time=100, walk_length=5):
        if structure_folder is not None and core_folder is not None:
            self.structure_generator = StructureInfoGenerator(base_path, origin_folder, structure_folder, core_folder, node_file, hop=hop)
        else:
...
def gcn_process(dataset, worker=-1):
    ...
    t1 = time.time()
    processing = Processing(base_path=base_path, origin_folder=origin_folder, structure_folder=None, core_folder=None,
                            walk_pair_folder='gcn_walk_pairs', node_freq_folder='gcn_node_freq', node_file=node_file,
                            hop=3, walk_time=10, walk_length=5)
...
def rwtgcn_process(dataset, worker=-1, calc_structure=True):
    ...
    t1 = time.time()
    processing = Processing(base_path=base_path, origin_folder=origin_folder, structure_folder='rwtgcn_structure_info', core_folder='rwtgcn_cores',
                            walk_pair_folder='rwtgcn_walk_pairs', node_freq_folder='rwtgcn_node_freq', node_file=node_file,
                            hop=5, walk_time=10, walk_length=5)
