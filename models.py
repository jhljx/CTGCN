...
class CTGCN(nn.Module):
    ...
    def forward(self, x_list, adj_list):
        ...
        #    hx = Variable(torch.zeros(batch_size, self.output_dim, device=device))
        ...
        # embedding = out[:, -1, :]
        # embedding = self.norm(out)
        # else:
        #     hx = Variable(torch.zeros(batch_size, self.output_dim, device=device))
        #     cx = Variable(torch.zeros(batch_size, self.output_dim, device=device))
        #
        #     for i in range(time_num):
        #         x = self.mlp_list[i](x_list[i])
        #         trans_list.append(x)
